Alias: $composition-clinicaldocument-versionNumber = http://hl7.org/fhir/StructureDefinition/composition-clinicaldocument-versionNumber
Alias: $condition-category = http://terminology.hl7.org/CodeSystem/condition-category
Alias: $condition-code = http://hl7.org/fhir/ValueSet/condition-code
Alias: $cpt = http://www.ama-assn.org/go/cpt
Alias: $data-absent-reason = http://hl7.org/fhir/StructureDefinition/data-absent-reason
Alias: $endpoint-connection-type = http://terminology.hl7.org/CodeSystem/endpoint-connection-type|2.1.1
Alias: $library-type = http://terminology.hl7.org/CodeSystem/library-type
Alias: $loinc = http://loinc.org
Alias: $observation-category = http://terminology.hl7.org/CodeSystem/observation-category
Alias: $patient-birthPlace = http://hl7.org/fhir/StructureDefinition/patient-birthPlace
Alias: $plan-definition-type = http://terminology.hl7.org/CodeSystem/plan-definition-type
Alias: $provider-taxonomy = http://nucc.org/provider-taxonomy
Alias: $resource-types = http://hl7.org/fhir/resource-types
Alias: $rxnorm = http://www.nlm.nih.gov/research/umls/rxnorm
Alias: $sct = http://snomed.info/sct
Alias: $umls = http://terminology.hl7.org/CodeSystem/umls
Alias: $us-ph-codesystem-usage-context = http://hl7.org/fhir/us/ph-library/CodeSystem/us-ph-codesystem-usage-context
Alias: $us-ph-codesystem-usage-context-type = http://hl7.org/fhir/us/ph-library/CodeSystem/us-ph-codesystem-usage-context-type
Alias: $usage-context-type = http://terminology.hl7.org/CodeSystem/usage-context-type
Alias: $v2-0203 = http://terminology.hl7.org/CodeSystem/v2-0203
Alias: $v2-0532 = http://terminology.hl7.org/CodeSystem/v2-0532
Alias: $v3-ActClass = http://terminology.hl7.org/CodeSystem/v3-ActClass
Alias: $v3-ActCode = http://terminology.hl7.org/CodeSystem/v3-ActCode
Alias: $v3-NullFlavor = http://terminology.hl7.org/CodeSystem/v3-NullFlavor
Alias: $v3-ObservationInterpretation = http://terminology.hl7.org/CodeSystem/v3-ObservationInterpretation
Alias: $v3-ParticipationType = http://terminology.hl7.org/CodeSystem/v3-ParticipationType
Alias: $v3-RoleCode = http://terminology.hl7.org/CodeSystem/v3-RoleCode
Alias: $v3-ServiceDeliveryLocationRoleType = http://terminology.hl7.org/ValueSet/v3-ServiceDeliveryLocationRoleType
Alias: $v3-TribalEntityUS = http://terminology.hl7.org/CodeSystem/v3-TribalEntityUS
Alias: $artifact-author = http://hl7.org/fhir/StructureDefinition/artifact-author
Alias: $valueset-expression = http://hl7.org/fhir/StructureDefinition/valueset-expression
Alias: $valueset-rules-text = http://hl7.org/fhir/StructureDefinition/valueset-rules-text
Alias: $WG-extension = http://hl7.org/fhir/StructureDefinition/structuredefinition-wg
Alias: $yes-no-unknown-not-asked = http://terminology.hl7.org/ValueSet/yes-no-unknown-not-asked
Alias: $system-us-core-category = http://hl7.org/fhir/us/core/CodeSystem/us-core-category
Alias: $system-observation-category = http://terminology.hl7.org/CodeSystem/observation-category
Alias: $v2-0532 = http://terminology.hl7.org/CodeSystem/v2-0532
Alias: $valueset-yes-no-unknown-not-asked = http://terminology.hl7.org/ValueSet/yes-no-unknown-not-asked
Alias: $iso3166-1-2 = http://hl7.org/fhir/ValueSet/iso3166-1-2
Alias: $valueset-act-class-exposure = http://terminology.hl7.org/ValueSet/v3-ActClassExposure
Alias: $valueset-exposure-setting = http://phinvads.cdc.gov/fhir/ValueSet/2.16.840.1.114222.4.11.7942
Alias: $valueset-exposure-location = http://phinvads.cdc.gov/fhir/ValueSet/2.16.840.1.114222.4.11.3209
Alias: $valueset-social-history-type = http://cts.nlm.nih.gov/fhir/ValueSet/2.16.840.1.113883.3.88.12.80.60
Alias: $valueset-us-core-condition = http://hl7.org/fhir/us/core/ValueSet/us-core-condition-code
Alias: $system-v2-0487 = http://terminology.hl7.org/CodeSystem/v2-0487
Alias: $system-jurisdiction-types = http://example.org/fhir/us/ph-library/CodeSystem/us-ph-codesystem-jurisdiction-types-example
Alias: $system-properties = http://example.org/fhir/CodeSystem/us-ph-codesystem-properties-example
Alias: $system-usps = https://www.usps.com/
Alias: $system-diagnosis-role = http://terminology.hl7.org/CodeSystem/diagnosis-role
// Alias: $shareable-library = http://hl7.org/fhir/StructureDefinition/shareablelibrary
// Alias: $shareable-plandefinition = http://hl7.org/fhir/StructureDefinition/shareableplandefinition
// Alias: $shareable-valueset = http://hl7.org/fhir/StructureDefinition/shareablevalueset
Alias: $shareable-library = http://hl7.org/fhir/uv/crmi/StructureDefinition/crmi-shareablelibrary
Alias: $shareable-plandefinition = http://hl7.org/fhir/uv/crmi/StructureDefinition/crmi-shareableplandefinition
Alias: $shareable-valueset = http://hl7.org/fhir/uv/crmi/StructureDefinition/crmi-shareablevalueset